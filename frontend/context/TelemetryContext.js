...
export function TelemetryProvider({ token, children }) {
  ...
  const refreshTelemetry = useCallback(async () => {
    ...
    try {
      ...
      if (!DISABLE_SENSOR_SANITY_CHECKS) {
        if (!Number.isFinite(rawTemp) || !Number.isFinite(rawHum)) {
        ...
        if (
          ...
          throw new Error(
            `Sensor reading out of expected range. Temp ${rawTemp.toFixed(1)}°C, RH ${rawHum.toFixed(1)}%.`
          );
        }
      }
      ...
      if (!DISABLE_SENSOR_SANITY_CHECKS && ageMs > STALE_READING_THRESHOLD_MS) {
      ...
      if (!DISABLE_SENSOR_SANITY_CHECKS) {
        ...
        // Suppress server-side alerts while bench-testing noisy/out-of-range values
